  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 p-6">
    ...
    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-6 mb-6">
      <div class="flex items-center justify-between mb-6">
                  <div>
            <label class="block text-white text-sm font-medium mb-2">Género</label>ex items-center gap-4">
          ...
          <div>
            <h1 class="text-2xl font-bold text-white">Gestión de Competidores</h1>
            <p class="text-blue-100">Administra competidores del sistema</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-4 gap-4">
        <div class="bg-white/10 rounded-lg p-4">
          <div class="text-2xl font-bold text-white">{{ competitors.length }}</div>
          <div class="text-blue-100 text-sm">Total</div>
        </div>
        <div class="bg-white/10 rounded-lg p-4">
          <div class="text-2xl font-bold text-green-400">{{ activeCount }}</div>
          <div class="text-blue-100 text-sm">Activos</div>
        </div>
        <div class="bg-white/10 rounded-lg p-4">
          <div class="text-2xl font-bold text-yellow-400">{{ inactiveCount }}</div>
          <div class="text-blue-100 text-sm">Inactivos</div>
        </div>
        <div class="bg-white/10 rounded-lg p-4">
          <div class="text-2xl font-bold text-purple-400">{{ filteredCompetitors.length }}</div>
          <div class="text-blue-100 text-sm">Filtrados</div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white/10 backdrop-blur-sm rounded-lg p-6 mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          <label class="block text-white text-sm font-medium mb-2">Club</label>
          <select 
        ...
        <table class="w-full">
          <thead class="bg-white/10">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-white font-medium">Club</th>
              <th class="px-6 py-4 text-left text-white font-medium">Categoría</th>
              <th class="px-6 py-4 text-left text-white font-medium">Peso</th>
              <th class="px-6 py-4 text-left text-white font-medium">Cinturón</th>
          ...
          <tbody class="divide-y divide-white/10">
            ...
            <tr v-else-if="filteredCompetitors.length === 0" class="text-white">
              <td colspan="8" class="px-6 py-8 text-center">
            ...
            <tr v-else v-for="competitor in paginatedCompetitors" :key="competitor.id" class="text-white hover:bg-white/5">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center">
                  ...
                  <div class="ml-3">
                    ...
                    <div class="text-gray-300 text-sm">{{ competitor.gender === 'MALE' ? 'Masculino' : 'Femenino' }}</div>
                  </div>
                </div>
              </td>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50" @click.self="closeModal">
      <div class="bg-gray-800 rounded-lg p-8 max-w-md w-full mx-4 max-h-screen overflow-y-auto">
        ...
        <form @submit.prevent="saveCompetitor" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-white text-sm font-medium mb-2">Nombres</label>
              <input 
            ...
            <div>
              <label class="block text-white text-sm font-medium mb-2">Apellidos</label>
              <input 
          ...
          <div>
            <label class="block text-white text-sm font-medium mb-2">Fecha de Nacimiento</label>
            <input 
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-white text-sm font-medium mb-2">Peso (kg)</label>
              <input 
            ...
            <div>
              <label class="block text-white text-sm font-medium mb-2">Altura (cm)</label>
              <input 
          ...
          <div>
            <label class="block text-white text-sm font-medium mb-2">Cinturón</label>
            <select 
              v-model="form.belt"
              required
              class="w-full bg-gray-700 text-white rounded-lg px-4 py-2 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            <label class="flex items-center space-x-2">
              ...
              >
              <span class="text-white text-sm">Competidor activo</span>
            </label>
          </div>
    ...
    <div v-if="showViewModal && selectedCompetitor" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50" @click.self="closeViewModal">
      <div class="bg-gray-800 rounded-lg p-8 max-w-2xl w-full mx-4 max-h-screen overflow-y-auto">
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-xl font-bold text-white">Detalles del Competidor</h3>
        ...
        <div class="grid grid-cols-2 gap-6">
          <div>
            <h4 class="text-lg font-semibold text-white mb-4">Información Personal</h4>
            <div class="space-y-3">
              <div>
                <span class="text-gray-300 text-sm">Nombre:</span>
                <p class="text-white font-medium">{{ selectedCompetitor.firstName }} {{ selectedCompetitor.lastName }}</p>
              </div>
              <div>
                <span class="text-gray-300 text-sm">Género:</span>
                <p class="text-white">{{ selectedCompetitor.gender === 'MALE' ? 'Masculino' : 'Femenino' }}</p>
              </div>
              <div>
                <span class="text-gray-300 text-sm">Fecha de Nacimiento:</span>
                <p class="text-white">{{ formatDate(selectedCompetitor.birthDate) }}</p>
              </div>
              <div>
                <span class="text-gray-300 text-sm">Edad:</span>
                <p class="text-white">{{ calculateAge(selectedCompetitor.birthDate) }} años</p>
              </div>
            </div>
          </div>
          
          <div>
            <h4 class="text-lg font-semibold text-white mb-4">Información Deportiva</h4>
            <div class="space-y-3">
              <div>
                <span class="text-gray-300 text-sm">Peso:</span>
                <p class="text-white">{{ selectedCompetitor.weight }} kg</p>
              </div>
              <div>
                <span class="text-gray-300 text-sm">Altura:</span>
                <p class="text-white">{{ selectedCompetitor.height }} cm</p>
              </div>
              <div>
                <span class="text-gray-300 text-sm">Cinturón:</span>
                <p class="text-white">{{ selectedCompetitor.belt }}</p>
              </div>
              <div>
                <span class="text-gray-300 text-sm">Club:</span>
                <p class="text-white">{{ selectedCompetitor.club?.name || 'Sin club' }}</p>
              </div>
              <div>
                <span class="text-gray-300 text-sm">Categoría:</span>
                <p class="text-white">{{ selectedCompetitor.category?.name || 'Sin categoría' }}</p>
              </div>
              <div>
...
const loadCompetitors = async () => {
  try {
    ...
    if (response.ok) {
      ...
      // Extraer los competidores del objeto de respuesta
      competitors.value = result.data?.competitors || []
    }
  } catch (err) {
