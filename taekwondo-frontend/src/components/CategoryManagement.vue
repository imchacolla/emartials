<template>
  <div class="min-h-screen bg-gray-50 p-6">
    ...
    <div class="bg-white shadow-lg rounded-lg p-6 mb-6 border border-gray-200">
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-gray-600">Administra categorías de competencia</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-4 gap-4">
        ...
        <div class="bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <div class="text-green-700 text-sm">Activas</div>
        </div>
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div class="text-2xl font-bold text-yellow-600">{{ inactiveCount }}</div>
          <div class="text-yellow-700 text-sm">Inactivas</div>
        </div>
        <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
          <div class="text-2xl font-bold text-purple-600">{{ filteredCategories.length }}</div>
          <div class="text-purple-700 text-sm">Filtradas</div>
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div>
          ...
          <input 
            ...
            type="text" 
            placeholder="Nombre..."
            class="w-full bg-white text-gray-900 rounded-lg px-4 py-2 border border-gray-300 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
        </div>
        ...
        <div>
          ...
          <select 
            ...
            class="w-full bg-white text-gray-900 rounded-lg px-4 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
        ...
        <div>
          <label class="block text-gray-700 text-sm font-medium mb-2">Estilo</label>
          <select 
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-200">
        ...
        <table class="w-full">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-gray-800 font-medium">Edad</th>
              <th class="px-6 py-4 text-left text-gray-800 font-medium">Peso</th>
              <th class="px-6 py-4 text-left text-gray-800 font-medium">Género</th>
              <th class="px-6 py-4 text-left text-gray-800 font-medium">Estilo</th>
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            <tr v-else-if="filteredCategories.length === 0" class="text-gray-800">
              <td colspan="10" class="px-6 py-8 text-center">
            ...
            <tr v-else v-for="category in paginatedCategories" :key="category.id" class="text-gray-800 hover:bg-gray-50">
              <td class="px-6 py-4">{{ category.id }}</td>
              <td class="px-6 py-4">
                <div class="font-medium">{{ category.name }}</div>
                <div class="text-gray-500 text-sm" v-if="category.belt">{{ category.belt }}</div>
              </td>
              ...
              <td class="px-6 py-4">
                <div class="text-center">
                  ...
                  <div class="text-gray-500 text-sm" v-if="category.maxParticipants">
      ...
      <div class="bg-gray-50 px-6 py-4 flex items-center justify-between border-t border-gray-200">
        ...
        <div class="flex space-x-2">
          ...
          <span class="px-3 py-1 bg-blue-600 text-gray-800 rounded">{{ currentPage }}</span>
          <button 
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50" @click.self="closeModal">
      <div class="bg-white shadow-xl border border-gray-200 rounded-lg p-8 max-w-2xl w-full mx-4 max-h-screen overflow-y-auto">
        <div class="flex justify-between items-center mb-6">
          ...
          <button @click="closeModal" class="text-gray-400 hover:text-gray-800">
        ...
        <form @submit.prevent="saveCategory" class="space-y-4">
          <div>
            <label class="block text-gray-800 text-sm font-medium mb-2">Nombre *</label>
            <input 
              v-model="form.name"
              type="text" 
              required
          ...
          <div>
            <label class="block text-gray-800 text-sm font-medium mb-2">Torneo *</label>
            <select 
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-gray-800 text-sm font-medium mb-2">Edad Mínima *</label>
              <input 
            ...
            <div>
              <label class="block text-gray-800 text-sm font-medium mb-2">Edad Máxima *</label>
              <input 
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-gray-800 text-sm font-medium mb-2">Peso Mínimo (kg) *</label>
              <input 
            ...
            <div>
              <label class="block text-gray-800 text-sm font-medium mb-2">Peso Máximo (kg) *</label>
              <input 
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-gray-800 text-sm font-medium mb-2">Género *</label>
              <select 
                v-model="form.gender"
                required
                class="w-full bg-white text-gray-800 rounded-lg px-4 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div>
              <label class="block text-gray-800 text-sm font-medium mb-2">Estilo *</label>
              <select 
                v-model="form.style"
                required
                class="w-full bg-white text-gray-800 rounded-lg px-4 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            <label class="block text-gray-800 text-sm font-medium mb-2">Cinturón/Grado</label>
            <input 
              v-model="form.belt"
              type="text" 
              class="w-full bg-white text-gray-800 rounded-lg px-4 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-gray-800 text-sm font-medium mb-2">Máximo Participantes</label>
              <input 
            ...
            <div>
              <label class="block text-gray-800 text-sm font-medium mb-2">Cuota de Inscripción</label>
              <input 
          ...
          <div>
            <label class="flex items-center space-x-2">
              <input 
                ...
                type="checkbox" 
                class="rounded bg-white border-gray-300 text-blue-600 focus:ring-blue-500 focus:ring-2"
              >
              <span class="text-gray-800 text-sm">Categoría activa</span>
            </label>
          </div>
          ...
          <div class="flex space-x-4 pt-4">
            <button 
              type="submit"
              :disabled="saving"
              class="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 text-gray-800 px-6 py-2 rounded-lg font-medium transition-colors duration-200"
            >
            ...
            <button 
              type="button"
              @click="closeModal"
              class="flex-1 bg-gray-600 hover:bg-white text-gray-800 px-6 py-2 rounded-lg font-medium transition-colors duration-200"
            >
              Cancelar
            </button>
          </div>
    ...
    <div v-if="showViewModal && selectedCategory" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50" @click.self="closeViewModal">
      <div class="bg-white shadow-xl border border-gray-200 rounded-lg p-8 max-w-3xl w-full mx-4 max-h-screen overflow-y-auto">
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-xl font-bold text-gray-800">Detalles de la Categoría</h3>
          <button @click="closeViewModal" class="text-gray-400 hover:text-gray-800">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
            </svg>
          </button>
          ...
          <div>
            ...
            <div class="space-y-3">
              <div>
                ...
                <p class="text-gray-800 font-medium">{{ selectedCategory.name }}</p>
              </div>
              <div>
                <span class="text-gray-500 text-sm">Torneo:</span>
                <p class="text-gray-800">{{ selectedCategory.tournament?.name || 'Sin torneo' }}</p>
              </div>
              <div>
                <span class="text-gray-500 text-sm">Edad:</span>
                <p class="text-gray-800">{{ selectedCategory.minAge }} - {{ selectedCategory.maxAge }} años</p>
              </div>
              <div>
                <span class="text-gray-500 text-sm">Peso:</span>
                <p class="text-gray-800">{{ selectedCategory.minWeight }}kg - {{ selectedCategory.maxWeight }}kg</p>
              </div>
              <div>
                <span class="text-gray-500 text-sm">Género:</span>
                <p class="text-gray-800">{{ getGenderText(selectedCategory.gender) }}</p>
              </div>
              <div>
                <span class="text-gray-500 text-sm">Estilo:</span>
                <p class="text-gray-800">{{ getStyleText(selectedCategory.style) }}</p>
              </div>
            </div>
          </div>
          
          <div>
            <h4 class="text-lg font-semibold text-gray-800 mb-4">Detalles Adicionales</h4>
            <div class="space-y-3">
              <div v-if="selectedCategory.belt">
                <span class="text-gray-500 text-sm">Cinturón/Grado:</span>
                <p class="text-gray-800">{{ selectedCategory.belt }}</p>
              </div>
              <div v-if="selectedCategory.maxParticipants">
                <span class="text-gray-500 text-sm">Máximo Participantes:</span>
                <p class="text-gray-800">{{ selectedCategory.maxParticipants }}</p>
              </div>
              <div v-if="selectedCategory.registrationFee">
                <span class="text-gray-500 text-sm">Cuota de Inscripción:</span>
                <p class="text-gray-800">${{ selectedCategory.registrationFee }}</p>
              </div>
              <div>
                <span class="text-gray-500 text-sm">Competidores Inscritos:</span>
                <p class="text-gray-800">{{ selectedCategory._count?.competitors || 0 }}</p>
              </div>
              <div>
                <span class="text-gray-500 text-sm">Registraciones:</span>
                <p class="text-gray-800">{{ selectedCategory._count?.registrations || 0 }}</p>
              </div>
              <div>
        ...
        <div class="mt-6 pt-4 border-t border-gray-700">
          <div class="flex space-x-4">
            <button 
              ...
              class="bg-blue-600 hover:bg-blue-700 text-gray-800 px-6 py-2 rounded-lg font-medium transition-colors duration-200"
            >
              Editar
            </button>
            <button 
              @click="closeViewModal"
              class="bg-gray-600 hover:bg-white text-gray-800 px-6 py-2 rounded-lg font-medium transition-colors duration-200"
            >
              Cerrar
            </button>
          </div>
